const priority = ["🔴", "🟡", "🔵"];
const categoryy = ["IITM","General"]

// ~ ILL LOOK INTO THIS LATER. 
// 🔴🔴 This is just for ths sake of testing right now thru insomnia. later ill shift this and see if i can use a trigger to initiate this.
...
// ~ 🟣 THIS WORKS PERFECTLY BUT ILL BE USING A GLOBAL VARAIABLE THAT CONTAINS THE ROW TITLES FROM WHERE I CAN ACCESS ALL OF THE INDEX TITLES, SINCE HERE ALL ATTRIBUTES ARE JUMBLED UP
function getUpcomingEvents() {
  var numDays = 7; // Number of days to consider for upcoming events
  var today = new Date();
  ...
  for (var i = 0; i < calendars.length; i++) {
    ...
    if ((calendarName == "Student Academic Calendar")||(calendarName == "Notion")||(calendarName == "Time Blocking")||(calendarName == "Deadlines")||(calendarName == "Reminders")||(calendarName == "Holidays in India")){continue;}
    else if (calendarName == "Birthdays") {prio=0;cate=1}
    else if (calendarName == "<events-from-ur-gmail>") {prio=1;cate=0}
    else {prio=2;cate=0}    
...
function getDataFromSheet() {
  var sheetId = "<yourown-majordatabase-id>"; // Replace with your actual Google Sheet ID
  var sheetName = "Central_Database"; // Replace with the name of the sheet from which you want to extract data
...
// ~ 🟣 CALLS THE ABOVE 2 FUNCTIONS AND THEN UPDATES THE MAJOR DATABASE AS PER THE DATA THAT IT RECIVES. 
// ~ ✅ APPEND THE HEADER TITLES OF THE COLUMNS TO THE SHEET AND MODIFY THE SEARCH OF THE isDuplicate TO ITTERATE FROM THE SECOND ROW ie INDEX 1
function updateNotionTable() {
  ...
  //  🟣~ THIS FOR LOOP CHECKS IF THE EVENTS ARE DUPLICATE AND IF NOT ADDS THE ROWS TO THE DATABASE
  // FROM THIS I WANT TO KEEP THE FOR LOOP, VERIFY FOR EACH ELEMENT ADD THE ONES THAT PASS TO A NEW ARRAY AND THEN PASS THE VALID ONES TO THE GSHEET. 
...
function extractRowsFromMajorDatabase() {
  ...
  // Old Col names
  // var columnNames = [
  //   "Task Title", "Description", "Category", "Task Type", "Priority", "This Month?",
  //   "This Week?", "Due Date", "Start Date", "Gcal ID", "Done Date", "Priority Gradient",
  //   "Task Id", "Randomizer","Classification","Prio Grad Value","NotionSheetsColab"
  // ];
  ...
  const spreadsheetId = '<your-minor-spreadsheetid>'; // Replace with your Google Sheets file ID
