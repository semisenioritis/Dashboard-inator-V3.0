...
function sendData() {
  ...
  fetch("<New-Task-Handler-Endpoint>", {
    redirect: "follow", 
    method: "POST",
...
function saveErrorToFile() {
  ...
  saveErrorButton.style.display = 'none';
    ...
    async function fetchAndSaveError() {
      try {
        const response = await fetch('https://example.com/data'); // Replace with your API endpoint
        const data = await response.json(); // Parse the response as JSON

        if (data.error) {
          // If the response contains an "error" field, show the "Save Error" button

          // Show the "Save Error" button
          var saveErrorButton = document.getElementById('saveErrorButton');
          saveErrorButton.style.display = 'inline';

          // Store the error data as a global variable to access it later in the saveErrorToFile() function
          window.errorData = data.error;
        } else {
          // Hide the "Save Error" button if no error
          const saveErrorButton = document.getElementById('saveErrorButton');
          saveErrorButton.style.display = 'none';
